...
// Session functions
function getAllSessions($status = 'active') {
    $db = getDB();
    if ($status === 'all') {
        $stmt = $db->prepare("SELECT * FROM spa_sessions ORDER BY name");
        $stmt->execute();
    } else {
        $stmt = $db->prepare("SELECT * FROM spa_sessions WHERE status = ? ORDER BY name");
        $stmt->execute([$status]);
    }
    return $stmt->fetchAll();
}
...
function getSessionById($id) {
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM spa_sessions WHERE id = ?");
    $stmt->execute([$id]);
    return $stmt->fetch();
}
...
function createSessionBooking($data) {
    ...
    try {
        $stmt = $db->prepare("
            INSERT INTO session_bookings (session_id, name, email, phone, spa_address, message, status)
            VALUES (?, ?, ?, ?, ?, ?, 'pending')
        ");
    ...
    return ['success' => false, 'message' => 'Session booking failed'];
}

function getAllSessionBookings() {
    $db = getDB();
    $stmt = $db->prepare("
